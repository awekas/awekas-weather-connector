...
let APIrequestIntervall = 15000; // Intervall to collect data in ms (15000 = 15s) 
...
class AwekasWeatherConnector extends utils.Adapter {
	...
	onReady() {
		...
		// get language from iiBroker		
		adapter.getForeignObject('system.config', (err, systemConfig) =>  {
        		language = systemConfig.common.language
				if(language != "de" && language != "en" && languega != "fr" && language != "es" && language != "nl" ){
					language = "en";
				}
				return language;		
		});
        ...
        //////intervalID = setInterval(requestData, 10000, adapter);
		setTimer(adapter);
...
function requestData(adapter){
		...
		if(adapter.config.token == ""){
			...
			//adapter.log.info('request: '+ url + adapter.config.token);
        	...
        	.then(function (response) {
            	if (response.status == 200) {
					...
					try{
						...
						adapter.setState("error", { val: APIerror, ack: true });
						if(APIerror == null){
							adapter.setState("fetchdate", { val: obj.fetchdate, ack: true });					
							adapter.setState("datatimestamp", { val: obj.current.datatimestamp, ack: true });
							adapter.setState("timeoffset", { val: obj.current.timeoffset, ack: true });
							adapter.setState("conditiontimestamp", { val: obj.current.conditiontimestamp, ack: true });
							adapter.setState("condition", { val: obj.current.condition, ack: true });
							
							// current values
							adapter.setState("current_temperature", { val: obj.current.temperature, ack: true });
							adapter.setState("current_dewpoint", { val: obj.current.dewpoint, ack: true });
							adapter.setState("current_windchill", { val: obj.current.windchill, ack: true });
							adapter.setState("current_wetbulbtemperature", { val: obj.current.wetbulbtemperature, ack: true });
							adapter.setState("current_humidity", { val: obj.current.humidity, ack: true });
							adapter.setState("current_airpress_rel", { val: obj.current.airpress_rel, ack: true });
							adapter.setState("current_tendency", { val: obj.current.tendency, ack: true });
							adapter.setState("current_precipitation", { val: obj.current.precipitation, ack: true });
							adapter.setState("current_rainrate", { val: obj.current.rainrate, ack: true });
							adapter.setState("current_itsraining", { val: obj.current.itsraining, ack: true });
							adapter.setState("current_windspeed", { val: obj.current.windspeed, ack: true });
							adapter.setState("current_gustspeed", { val: obj.current.gustspeed, ack: true });
							adapter.setState("current_winddirection", { val: obj.current.winddirection, ack: true });
							let winddirtext = winddirText(obj.current.winddirection);
							adapter.setState("current_winddirection_text", { val: winddirtext, ack: true });
							adapter.setState("current_uv", { val: obj.current.uv, ack: true });
							adapter.setState("current_solar", { val: obj.current.solar, ack: true });
							adapter.setState("current_brightness", { val: obj.current.brightness, ack: true });
							adapter.setState("current_suntime", { val: obj.current.suntime, ack: true });
							adapter.setState("snowheighttimestamp", { val: obj.current.snowheighttimestamp, ack: true });
							adapter.setState("snowheight", { val: obj.current.snowheight, ack: true });
							adapter.setState("current_temp1", { val: obj.current.temp1, ack: true });
							adapter.setState("current_temp2", { val: obj.current.temp2, ack: true });
							adapter.setState("current_temp3", { val: obj.current.temp3, ack: true });
							adapter.setState("current_temp4", { val: obj.current.temp4, ack: true });
							adapter.setState("current_humidity1", { val: obj.current.humidity1, ack: true });
							adapter.setState("current_humidity2", { val: obj.current.humidity2, ack: true });
							adapter.setState("current_humidity3", { val: obj.current.humidity3, ack: true });
							adapter.setState("current_humidity4", { val: obj.current.humidity4, ack: true });
							adapter.setState("current_soilmoisture1", { val: obj.current.soilmoisture1, ack: true });
							adapter.setState("current_soilmoisture2", { val: obj.current.soilmoisture2, ack: true });
							adapter.setState("current_soilmoisture3", { val: obj.current.soilmoisture3, ack: true });
							adapter.setState("current_soilmoisture4", { val: obj.current.soilmoisture4, ack: true });
							adapter.setState("current_leafwetness1", { val: obj.current.leafwetness1, ack: true });
							adapter.setState("current_leafwetness2", { val: obj.current.leafwetness2, ack: true });
							adapter.setState("current_indoortemperature", { val: obj.current.indoortemperature, ack: true });
							adapter.setState("current_indoorhumidity", { val: obj.current.indoorhumidity, ack: true });
							adapter.setState("current_airquality_pm1", { val: obj.current.airquality_pm1, ack: true });
							adapter.setState("current_airquality_pm2", { val: obj.current.airquality_pm2, ack: true });
							adapter.setState("current_airquality_pm10", { val: obj.current.airquality_pm10, ack: true });

							// 1h values
							adapter.setState("1h_precipitation", { val: obj["1h"]["precipitation_1h"] , ack: true });
							
							// values for current da
							adapter.setState("day_temp_min", { val: obj.day.temp_min , ack: true });
							adapter.setState("day_temp_min_ts", { val: obj.day.temp_min_ts , ack: true });
							adapter.setState("day_temp_max", { val: obj.day.temp_max , ack: true });
							adapter.setState("day_temp_max_ts", { val: obj.day.temp_max_ts , ack: true });
							adapter.setState("day_dewpoint_min", { val: obj.day.dewpoint_min , ack: true });
							adapter.setState("day_dewpoint_min_ts", { val: obj.day.dewpoint_min_ts , ack: true });
							adapter.setState("day_dewpoint_max", { val: obj.day.dewpoint_max , ack: true });
							adapter.setState("day_dewpoint_max_ts", { val: obj.day.dewpoint_max_ts , ack: true });
							adapter.setState("day_hum_min", { val: obj.day.hum_min , ack: true });
							adapter.setState("day_hum_min_ts", { val: obj.day.hum_min_ts , ack: true });
							adapter.setState("day_hum_max", { val: obj.day.hum_max , ack: true });
							adapter.setState("day_hum_max_ts", { val: obj.day.hum_max_ts , ack: true });
							adapter.setState("day_airp_rel_min", { val: obj.day.airp_rel_min , ack: true });
							adapter.setState("day_airp_rel_min_ts", { val: obj.day.airp_rel_min_ts , ack: true });
							adapter.setState("day_airp_rel_max", { val: obj.day.airp_rel_max , ack: true });
							adapter.setState("day_airp_rel_max_ts", { val: obj.day.airp_rel_max_ts , ack: true });
							adapter.setState("day_windspeed_min", { val: obj.day.windspeed_min , ack: true });
							adapter.setState("day_windspeed_min_ts", { val: obj.day.windspeed_min_ts , ack: true });
							adapter.setState("day_windspeed_max", { val: obj.day.windspeed_max , ack: true });
							adapter.setState("day_windspeed_max_ts", { val: obj.day.windspeed_max_ts , ack: true });
							adapter.setState("day_winddir_max", { val: obj.day.winddir_max , ack: true });
							let winddir_max_text = winddirText(obj.day.winddir_max);
							adapter.setState("day_winddir_max_text", { val: winddir_max_text , ack: true });
							adapter.setState("day_gustspeed_min", { val: obj.day.gustspeed_min , ack: true });
							adapter.setState("day_gustspeed_min_ts", { val: obj.day.gustspeed_min_ts , ack: true });
							adapter.setState("day_gustspeed_max", { val: obj.day.gustspeed_max , ack: true });
							adapter.setState("day_gustspeed_max_ts", { val: obj.day.gustspeed_max_ts , ack: true });
							adapter.setState("day_gustdir_max", { val: obj.day.gustdir_max , ack: true });
							let gustdir_max_text = winddirText(obj.day.gustdir_max);
							adapter.setState("day_gustdir_max_text", { val: gustdir_max_text , ack: true });
							adapter.setState("day_rainrate_max", { val: obj.day.rainrate_max , ack: true });
							adapter.setState("day_rainrate_max_ts", { val: obj.day.rainrate_max_ts , ack: true });
							adapter.setState("day_precipitation_24h", { val: obj.day.precipitation_24h , ack: true });
							adapter.setState("day_brightness_max", { val: obj.day.brightness_max , ack: true });
							adapter.setState("day_brightness_max_ts", { val: obj.day.brightness_max_ts , ack: true });
							adapter.setState("day_solar_max", { val: obj.day.solar_max , ack: true });
							adapter.setState("day_solar_max_ts", { val: obj.day.solar_max_ts , ack: true });
							adapter.setState("day_uv_max", { val: obj.day.uv_max , ack: true });
							adapter.setState("day_uv_max_ts", { val: obj.day.uv_max_ts , ack: true });
							adapter.setState("day_intemp_min", { val: obj.day.intemp_min , ack: true });
							adapter.setState("day_intemp_min_ts", { val: obj.day.intemp_min_ts , ack: true });
							adapter.setState("day_intemp_max", { val: obj.day.intemp_max , ack: true });
							adapter.setState("day_intemp_max_ts", { val: obj.day.intemp_max_ts , ack: true });
							adapter.setState("day_inhum_min", { val: obj.day.inhum_min , ack: true });
							adapter.setState("day_inhum_min_ts", { val: obj.day.inhum_min_ts , ack: true });
							adapter.setState("day_inhum_max", { val: obj.day.inhum_max , ack: true });
							adapter.setState("day_inhum_max_ts", { val: obj.day.inhum_max_ts , ack: true });
							adapter.setState("day_airquality_pm1", { val: obj.day.airquality_pm1 , ack: true });
							adapter.setState("day_airquality_pm2", { val: obj.day.airquality_pm2 , ack: true });
							adapter.setState("day_airquality_pm10", { val: obj.day.airquality_pm10 , ack: true });

							// forecast Day 0
							adapter.setState("forecast_day0_code", { val: obj.forecast.day0.fc_code , ack: true });
							adapter.setState("forecast_day0_icon", { val: obj.forecast.day0.fc_icon , ack: true });
							adapter.setState("forecast_day0_text", { val: obj.forecast.day0.fc_text , ack: true });
							adapter.setState("forecast_day0_temp_min", { val: obj.forecast.day0.fc_temp_min , ack: true });
							adapter.setState("forecast_day0_temp_max", { val: obj.forecast.day0.fc_temp_max , ack: true });
							adapter.setState("forecast_day0_rainsum", { val: obj.forecast.day0.fc_rainsum , ack: true });
							adapter.setState("forecast_day0_rain_possibility", { val: obj.forecast.day0.fc_rain_possibility , ack: true });

							// forecast Day 1
							adapter.setState("forecast_day1_code", { val: obj.forecast.day1.fc_code , ack: true });
							adapter.setState("forecast_day1_icon", { val: obj.forecast.day1.fc_icon , ack: true });
							adapter.setState("forecast_day1_text", { val: obj.forecast.day1.fc_text , ack: true });
							adapter.setState("forecast_day1_temp_min", { val: obj.forecast.day1.fc_temp_min , ack: true });
							adapter.setState("forecast_day1_temp_max", { val: obj.forecast.day1.fc_temp_max , ack: true });
							adapter.setState("forecast_day1_rainsum", { val: obj.forecast.day1.fc_rainsum , ack: true });
							adapter.setState("forecast_day1_rain_possibility", { val: obj.forecast.day1.fc_rain_possibility , ack: true });
							
							// forecast Day 2
							adapter.setState("forecast_day2_code", { val: obj.forecast.day2.fc_code , ack: true });
							adapter.setState("forecast_day2_icon", { val: obj.forecast.day2.fc_icon , ack: true });
							adapter.setState("forecast_day2_text", { val: obj.forecast.day2.fc_text , ack: true });
							adapter.setState("forecast_day2_temp_min", { val: obj.forecast.day2.fc_temp_min , ack: true });
							adapter.setState("forecast_day2_temp_max", { val: obj.forecast.day2.fc_temp_max , ack: true });
							adapter.setState("forecast_day2_rainsum", { val: obj.forecast.day2.fc_rainsum , ack: true });
							adapter.setState("forecast_day2_rain_possibility", { val: obj.forecast.day2.fc_rain_possibility , ack: true });

							// forecast Day 3
							adapter.setState("forecast_day3_code", { val: obj.forecast.day3.fc_code , ack: true });
							adapter.setState("forecast_day3_icon", { val: obj.forecast.day3.fc_icon , ack: true });
							adapter.setState("forecast_day3_text", { val: obj.forecast.day3.fc_text , ack: true });
							adapter.setState("forecast_day3_temp_min", { val: obj.forecast.day3.fc_temp_min , ack: true });
							adapter.setState("forecast_day3_temp_max", { val: obj.forecast.day3.fc_temp_max , ack: true });
							adapter.setState("forecast_day3_rainsum", { val: obj.forecast.day3.fc_rainsum , ack: true });
							adapter.setState("forecast_day3_rain_possibility", { val: obj.forecast.day3.fc_rain_possibility , ack: true });

							// forecast Day 4
							adapter.setState("forecast_day4_code", { val: obj.forecast.day4.fc_code , ack: true });
							adapter.setState("forecast_day4_icon", { val: obj.forecast.day4.fc_icon , ack: true });
							adapter.setState("forecast_day4_text", { val: obj.forecast.day4.fc_text , ack: true });
							adapter.setState("forecast_day4_temp_min", { val: obj.forecast.day4.fc_temp_min , ack: true });
							adapter.setState("forecast_day4_temp_max", { val: obj.forecast.day4.fc_temp_max , ack: true });
							adapter.setState("forecast_day4_rainsum", { val: obj.forecast.day4.fc_rainsum , ack: true });
							adapter.setState("forecast_day4_rain_possibility", { val: obj.forecast.day4.fc_rain_possibility , ack: true });

							// forecast Day 5
							adapter.setState("forecast_day5_code", { val: obj.forecast.day5.fc_code , ack: true });
							adapter.setState("forecast_day5_icon", { val: obj.forecast.day5.fc_icon , ack: true });
							adapter.setState("forecast_day5_text", { val: obj.forecast.day5.fc_text , ack: true });
							adapter.setState("forecast_day5_temp_min", { val: obj.forecast.day5.fc_temp_min , ack: true });
							adapter.setState("forecast_day5_temp_max", { val: obj.forecast.day5.fc_temp_max , ack: true });
							adapter.setState("forecast_day5_rainsum", { val: obj.forecast.day5.fc_rainsum , ack: true });
							adapter.setState("forecast_day5_rain_possibility", { val: obj.forecast.day5.fc_rain_possibility , ack: true });
							
							// check timer if in waiting state 
							if (timer == 300000){
								timer = APIrequestIntervall;
								setTimer(adapter);
								adapter.log.info('AWEKAS Server connection restored. set request timer to normal');
							}
							
						}else{
							// station API error handling 
							if (APIerror == "maximum quota exceeded"){
								adapter.log.warn('maximum quota exceeded - to many requests. waiting 5min...');
								timer = 300000;
								setTimer(adapter);
							}
							if (APIerror == "AWEKAS plus not active"){
								adapter.log.warn('AWEKAS Plus not active - AWEKAS Plus is necessary for station API- please activate AWEKAS Plus in AWEKAS user profile.');
								timer = 300000;
								setTimer(adapter);
							}
							if (APIerror =="invalid key"){
								adapter.log.warn('Invalid AWEKAS API key - please get the correct key from https://my.awekas.at ');
								timer = 300000;
								setTimer(adapter);
							}
						}
...
// function set intervall for request
function setTimer(adapter){
	clearInterval(intervalID);
	intervalID = setInterval(requestData, timer, adapter);
}

// function transform deggree to winddirection in selected language
function winddirText(richtung){
